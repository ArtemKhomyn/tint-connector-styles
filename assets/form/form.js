                ...
                function render() {
                    ...
                    if(!btn) {
                        ...
                        document.querySelector('.tint_index_btn')?.addEventListener('click', (e) => {
                            ...
                            currentPost = {
                                id: fullDataPosts[postsIndexValue.value - 1]?.id,
                            ...
                            if(currentPost.Postimage || currentPost.user) {
                                ...
                                title.innerHTML = `<b><i>Post User - </i></b><u>${currentPost.user}</u>, <b><i>Post ID - </i>${currentPost.id}</b>`;
                                ...
                                const title = document.createElement('p');
                                title.classList.add('index_title')
                                title.innerHTML = `<b><i>Range of posts is from 1 to ${fullDataPosts.length}</b></i>`;
                                postsIndexWrap.appendChild(title);
                                const img = document.createElement('img');
                                img.classList.add('index_image')
                                img.src = '';
