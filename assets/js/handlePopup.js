...
function createPopupSlider(slideIndex, fullDataPosts, additionalCTA, fullProductsData, shareBtn, tint_user_settings) {  
  ...
  // Datalayer post click
  if (typeof window.tintPostClickDl === 'function') {
    const currentPost = fullDataPosts[slideIndex];
    window.tintPostClickDl(currentPost);
  }
  ...
  const popupSlides = fullDataPosts.map((post, i) => {
    ...
    const isProductTag = post.relationships.product_tags.data || [];
      ...
      const defineCTA = (id, cta) => {        
        const ctaElement = cta.filter(elem => elem.post_id === id);
          ...
          <div>   
              ...
              <div class="tint_c_products">
                ${additionalProducts.map((product, index) => {                  
                  return (
                    `
                      <div 
                        class='tint_c_products_item' 
                        data-post-index='${i}' 
                        data-product-index='${index}'
                        data-product-id='${product?.id}'
                        data-product-title='${product?.attributes?.name}'
                        data-product-url='${product?.attributes?.url}'
                        data-product-sku='${product?.attributes?.sku}'
                      >
                        <a       
                          target=${target_blank}                   
            ...
            <div class="popup_slide-bottom">
              <div class="bottom_cta-wrap">
                ...
                <a class="bottom_cta bottom_cta_buy_now ${post.relationships.cta_associations.meta.included ? 'show' : 'hide_btn'}" target=${target_blank} href='#' : ''} data-post-index=${i}>
                  ${Drupal.t('Info about brand')}
                </a>
                ${defineCTA(post.id, additionalCTA)}
              </div>
  ...
  const postCta = document.querySelectorAll('.popup_slide-content .bottom_cta_buy_now');
  
  postCta.forEach(button => {
    button.addEventListener('click', (e) => {
      const buttonText = e.target.innerText;
      const href = e.target.getAttribute('href');
      const currentPostIndex = e.target.getAttribute('data-post-index');

      const buttonData = {
        text: buttonText,
        link: href,
        currentPost: fullDataPosts[currentPostIndex]
      }

      window.clickBuyNow(buttonData);
    })
  })
  const productLD = document.querySelectorAll('.popup_slide-content .tint_c_products_item');
  productLD.forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const productSku =  e.target.getAttribute('data-product-sku');
      ...
      window.clickTintProducts(productData);
    })
  })

  const additionalCtaButton = document.querySelectorAll('.popup_slide-content .bottom_cta_content')

  additionalCtaButton.forEach(button => {
    button.addEventListener('click', (e) => {
      const ctaUrl =  e.target.getAttribute('href');
      const ctaId =  e.target.getAttribute('data-post-id');
      ...
      window.clickTintAdditionalCta(productData);
    })
  })
};
